...
public class Util {
  ...
  public void setCondenser(Block b, String p) {
    ...
    if ((cb = getConv(x)) != null) {
      ItemStack t = new ItemStack(Material.AIR, -1);
  ...
  public int getEfficiency(Block[] cb) {
    if (cb[2].getType().equals(main.m1))
      return 1;
    if (cb[2].getType().equals(main.m2))
      return 2;
    if (cb[2].getType().equals(main.m3))
      return 3;
    if (cb[2].getType().equals(main.m4))
      return 4;
    return 0;
  }
  ...
  public void stringToCondenser(String s) {
    ...
    if (w != null) {
      ...
      if (cBlocks(blocks)) {
        ItemStack t = new ItemStack(Material.getMaterial(p[6]), 1, Short.parseShort(p[7]));
  ...
  public void breakCondenser(Condenser c) {
    ...
    // i.addItem((new ItemStack(54, 1)), (new ItemStack(116, 1)), (new ItemStack(42,
    // 1)), (new ItemStack(49, 1)),
    // (new ItemStack(m, 1)));
    // }
  }
  ...
  public Condenser getCondenser(Block b) {
    ...
    } else if (b.getType() == Material.ENCHANTING_TABLE) {
      for (String s : main.con.keySet()) {
        Condenser c = main.con.get(s);
    ...
    } else if (b.getType() == Material.ACACIA_SIGN) {
      for (String s : main.con.keySet()) {
        Condenser c = main.con.get(s);
  ...
  public Block getBase(Block b) {
    if (b.getType() == Material.IRON_BLOCK) {
      for (int x = 1; x < 5; x++) {
        Block b2 = lFO(b, x, 2);
        Block b3 = lFO(b, x, 1);
        if ((main.varBase.contains(b2.getType())) && (b3.getType() == Material.OBSIDIAN))
          return b;
      }
    } else if (b.getType() == Material.OBSIDIAN) {
      for (int x = 1; x < 5; x++) {
        ...
        if ((main.varBase.contains(b2.getType())) && (b3.getType() == Material.IRON_BLOCK))
          return b3;
      }
      ...
      for (int x = 1; x < 5; x++) {
        Block b2 = lFO(b, x, 1);
        Block b3 = lFO(b, x, 2);
        if ((b2.getType() == Material.OBSIDIAN) && (b3.getType() == Material.IRON_BLOCK))
          return b3;
      }
    } else if (b.getType() == Material.CHEST) {
      Block u = b.getWorld().getBlockAt(b.getX(), b.getY() - 1, b.getZ());
      if (u.getType() == Material.IRON_BLOCK)
        return u;
  ...
  public boolean cBlocks(Block[] a) {
    return (a[0].getType() == Material.IRON_BLOCK) && (a[1].getType() == Material.OBSIDIAN)
        ...
        && (a[4].getType() == Material.ENCHANTING_TABLE) && (a[5].getType() == Material.CHEST);
  }
