...
public class Main extends JavaPlugin {
  ...
  public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args) {
    if (cmd.getName().equalsIgnoreCase("emc") && (sender instanceof Player)) {
      ...
      p.sendMessage("The EMC value of: " + Main.instance.conf.getName(p.getInventory().getItemInMainHand(), false)
          + " is " + Main.instance.conf.getEMC(p.getInventory().getItemInMainHand()));
      return true;
    } else if (cmd.getName().equalsIgnoreCase("ec")) {
      if (args.length == 0) {
        sender.sendMessage("/ec reload: Reload EnergyCraft config.");
      } else {
        if (args[0].equalsIgnoreCase("reload")) {
          if (sender instanceof Player) {
            ...
            if (p.hasPermission("ec.reload")) {
              ...
              // saveConfig();
              reloadConfig();
              sender.sendMessage("[EnergyCraft Reloaded] Config Reloaded!!");
              return true;
            }
          } else {
            conf.loadEMCConfig();
  ...
  private void loadConfig() {
    ...
    baseblocks.add(Material.ENCHANTING_TABLE);
