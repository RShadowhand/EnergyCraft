...
public class Main extends JavaPlugin {
	...
	public void onEnable() {
		...
		getServer().getScheduler().scheduleSyncRepeatingTask(this,
				new CondenseTask(), 0L, checkInterval * 5L);
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd,
		...
		if (cmd.getName().equalsIgnoreCase("emc") && (sender instanceof Player)) {
			...
			p.sendMessage("The EMC value of: "
					+ Main.instance.conf.getName(p.getItemInHand(), false)
					+ " is " + Main.instance.conf.getEMC(p.getItemInHand()));
			return true;
